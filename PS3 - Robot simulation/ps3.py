...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        self.tiles[tile] = max(self.tiles[tile] - capacity, 0)
    ...
    def is_position_in_room(self, pos):
        ...
        """
        x_in_room = pos.get_x() >= 0 and pos.get_x() < self.width
        y_in_room = pos.get_y() >= 0 and pos.get_y() < self.height
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        unfurnished_tiles = self.tiles.copy()
        for furnished_tile in self.furniture_tiles:
            unfurnished_tiles.pop(furnished_tile)
        chosen_tile = random.choice(list(unfurnished_tiles.keys()))
        x = random.uniform(chosen_tile[0], chosen_tile[0] + 1)
        y = random.uniform(chosen_tile[1], chosen_tile[1] + 1)
        return Position(x, y)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    robot_type: class of robot to be instantiated (e.g. StandardRobot or
                FaultyRobot)
    """
    trial_steps = [] #records the number of steps required in each trial, will be used to calculate avg
    total_tiles = width*height
    for i in range(num_trials):
        robot_list = [] #the robots operating in the trial
        fraction_cleaned = 0 #fraction of tiles that are completely clean; 0 to start with
        ...
        while fraction_cleaned < min_coverage:
            ...
            fraction_cleaned = room.get_num_cleaned_tiles()/total_tiles
            num_of_time_steps += 1
        trial_steps.append(num_of_time_steps)
        
    return sum(trial_steps)/num_trials
...
#       of a 20x20 room?
#   
#   Using the following code we see that the standard robot takes on average about 2000 steps 
#   compared to 2500 steps for the faulty robot
# =============================================================================
...
#
#       room dimensions: avg time steps standard robot vs average time steps faulty robot
#       10x30: 791 vs 986
#       20x15: 784 vs 952
#       25x12: 794 vs 970
#       50x6: 823 vs 1211
#       In the first three rooms the robots exhibit similar performance, but in the last
#       their performance is notably worse. This could be because the room is narrower so
#       the robots are having to rotate more often to remain in the room, which takes up a time-step each time
# =============================================================================
